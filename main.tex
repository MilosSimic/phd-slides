...
\begin{frame}
	...
	\begin{itemize}
		...
		\end{enumerate}
	\item They are crucial for understanding the concept of \textbf{distributed clouds -- $\mu$ clouds}
	\end{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{enumerate}
			...
			\item \textbf{Lack of a global clock} -- each node has its notion of time
		\end{enumerate}
	\end{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Clouds provide services, clients use them to avoid \textbf{huge} investments -- creating and maintaining \textbf{big} data centers (\textbf{expensive})
		\item Clients \textbf{only} pay for their usage time -- \emph{pay as you go model}
	...
	\begin{columns}
		\begin{column}{0.5\linewidth}
			\begin{center}
				...
				\quickCite{Outside Google Data center}
			\end{center}
		\end{column}
		\begin{column}{0.5\linewidth}
			\begin{center}
				\includegraphics[width=1\linewidth]{images/clusters}
				\quickCite{Inside Google Data center}
			\end{center}
		\end{column}
	\end{columns}
	
	\emph{And we need to have the way to program these groups of machines}
...
\begin{frame}
	...
	\begin{itemize}
		\item Centralization \textbf{demands} to move all data to the cloud --- big issue
		\begin{itemize}
		...
		\item Some sensitive data \textbf{cannot} be moved out of the state border
...
\begin{frame}
	...
	\begin{itemize}
		\item The cloud is usually far away from end devices -- \textbf{data centers are built on specific locations} (target as many users nearby as possible)
...
\begin{frame}
	...
	\begin{itemize}
		\item A model where computing and storage utilities are \textbf{in proximity} to data sources
		\item It introduced \textbf{small-scale servers} -- operate \textbf{between data sources and the cloud}
		...
		\end{itemize}
		\item To avoid latency and huge bandwidth, these servers can be dispersed in various locations
...
\begin{frame}
	...
	\end{itemize}
	\item Processing (Big Data):
	\begin{itemize}
		\item Data locality -- moving computation to the data, instead of vice versa (cloud)
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Forming micro clouds (pools of resources) depends on local population \textbf{usage and needs} -- adaptive
		\item Reduce traditional data centers fixed costs with \textbf{agility} -- dynamically grow and shrink resources as needed
	\end{itemize}
...
\begin{frame}
	...
	\begin{figure}[H]
		\begin{center}
			...
			\quickCite{Edge-centric computing as a service architecture with separation of concerns.}
		\end{center}
	\end{figure}
\end{frame}
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.5\linewidth}
			\begin{itemize}
				\item \textbf{Ephemeral cloud-like structures} serving local requests first, before reaching to the traditional cloud 
				\item Size and existence are defined by the \textbf{local population's needs}
				\item Clients have the \textbf{illusion} to communicate with the clouds
				\item Designed for \textbf{failure} using automated tools
				\item \textbf{Collaborate} with traditional clouds -- biologically inspired computing
			\end{itemize}
		\end{column}
		\begin{column}{0.5\linewidth}
			\begin{center}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item They can serve as firewalls and pre-processing tier for the cloud (smart (big) data)
...
\begin{frame}
	...
	To be a part of the system, a node \textbf{must} satisfy four simple rules:
	\begin{enumerate}
		\item Run an operating system with a file system
		\item Be able to run some application isolation tool (e.g. container or unikernel engine)
	...
	\begin{itemize}
		\item Simple yet powerful rules are here to help -- increased demand for resources that the currently available infrastructure \textbf{cannot} support
...
\begin{frame}
	...
	\begin{itemize}
		\item Nodes can be \textbf{heterogeneous} by their nature
		...
		\begin{itemize}
			%
			%
			\item $L$ is a set of ordered \emph{key-value} pairs representing node \emph{labels} or \textbf{server-specific features} (e.g., os:linux, arch:arm, isolation:containers, etc.) 
			%
			%
			\item $R$ is representing node resources (kind, total, free, used, etc.)
			%
			%
...
\begin{frame}
	...
	\begin{itemize}
		\item Nodes are organized into micro clouds dynamically, \textbf{by abstracting infrastructure to the level of software} --- infrastructure as software
		\item The proposed system relies on \textbf{four} protocols:
		\bigskip 
		\begin{enumerate}
			\item \textbf{healthcheck} protocol informs the system about the state of every node --- background operation
			\item \textbf{cluster formation} protocol forms new clusters --- mutate operation (atomic, immutable, and idempotent)
			\item \textbf{Idempotency check} protocol prevent mutate operation \textbf{if} infrastructure already exists
			\item \textbf{list detail} protocol shows the current state of the system to the user --- list operation
		\end{enumerate}
	\end{itemize}
...
\begin{frame}
	\frametitle{Healthcheck protocol}
	
	\begin{columns}
		\begin{column}{0.5\linewidth}
			\begin{itemize}
				\item The \textbf{order} in which messages arrive is \textbf{not} important
				\item The important part is that every message \textbf{eventually} comes into the system
				\item All nodes are \textbf{equal}, part of some cluster or not
				\item Formally adding node to the system \textbf{if not present}:
				\begin{equation*}
...
\begin{frame}
	\frametitle{Cluster formation and Idempotency check protocols}
	
	\begin{columns}
		\begin{column}{0.5\linewidth}
			\begin{itemize}
				\item Pick desired nodes, rules:
				\begin{itemize}
					\item $i_\mathit{th}$ server's labels set $s_i[L]$ and the query selector $Q$ are same size
					\begin{equation*}
					...
					\end{align*}
				\end{itemize}
				\item Check idempotency
				\item Reserve nodes for some time $T_r$
				\item Nodes start \textbf{membership protocol}
			\end{itemize}
		\end{column}
		\begin{column}{0.5\linewidth}
			\begin{center}
				\begin{figure}
					\includegraphics<1>[scale=0.55]{images/FIG31}
					\includegraphics<2>[scale=0.55]{images/FIG3}
				\end{figure}
			\end{center}
		\end{column}
	\end{columns}
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.6\linewidth}
			\begin{center}
				If there is a bug in a \emph{distributed algorithm}, \\
				no matter how improbable it may seem, \\
				it's not a question of \textbf{whether} it will appear, \\
				it's a question of \textbf{when} it will appear.
				\newline
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.5\linewidth}
			...
			\begin{itemize}
				\item \textbf{a stand-alone} implementation --- using open source tools
				\item \textbf{integration} within existing tools, as a node organizer and register (e.g. Kubernetes node organizator)
			\end{itemize}
		
			\smallskip
		
			\begin{itemize}
				\item \textbf{Do not} lock users in specific provider ecosystem
			\end{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		\item Infrastructure deployment will \textbf{not happen overnight} -- it might take years
		\item It might not be started at all until the whole process is \emph{trivial} -- \textbf{key is to simplify remote micro clouds management}
		\item The naive approach, do it manually -- super tedious and time-consuming
		\item \textbf{The infrastructure needs to be constantly deployed and maintained} -- automation is the key
...
\begin{frame}
	...
	\begin{itemize}
		\item Abstracting infrastructure to the level of software -- \textbf{infrastructure as software} (already available tools, principles, and techniques)
		\item Micro clouds model is \textbf{movable} in between edge and cloud -- do we want more cloud-like or edge-like system
...
\begin{frame}
	...
	\begin{columns}
		\begin{column}{0.5\linewidth}
			\begin{itemize}
				\item Specify \textbf{desired state} using \emph{YAML} or some other format -- descriptively
				\item Submit \textbf{desired state} to the system, and \textbf{let the system deal with the rest}
	...
	\begin{columns}
		\begin{column}{0.5\linewidth}
			\begin{itemize}
				...
				\item Every node \textbf{must} provide current state, for comparison -- \textbf{healthcheck} protocol
		...
		\begin{column}{0.5\linewidth}
			\begin{center}
				\begin{figure}
					\includegraphics<1>[scale=0.6]{images/Figure3}
					\includegraphics<2>[scale=0.6]{images/Figure30}
				\end{figure}
			\end{center}
		\end{column}
	\end{columns}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Allowing mIaaS, mCaaS, mPaaS, mSaaS models closer to the data data sources
	\end{itemize}
	...
	\begin{columns}
		\begin{column}{0.5\linewidth}
			...
			\begin{itemize}
				...
				\item Eventually -- OS capable of running infrastructure with \textbf{less to none} human intervention
			\end{itemize}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item Serving local requests first, and from optimal location
		\item Abstracted infrastructure to the level of software -- infrastructure as software
		\item Moving computation to the node where that data resides, instead of vice versa
	\end{itemize} 
